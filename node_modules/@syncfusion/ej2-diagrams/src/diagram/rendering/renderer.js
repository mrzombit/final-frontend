        ...
        if (isSegmentEditing) {
            if ((selector.type === 'Straight' || selector.type === 'Bezier') && selector.segments.length > 0) {
                for (i = 0; i < selector.segments.length - 1; i++) {
                    ...
                    this.renderCircularHandle(('segementThumb_' + (i + 1)), wrapper, segment.point.x, segment.point.y, canvas, true, constraints & ThumbsConstraints.ConnectorSource, transform, connectedSource, null, null, i, null, handleSize);
                }
            }
            else {
        ...
        if (selector.type === 'Bezier' && canShowBezierPoints) {
            ...
            for (i = 0; i <= segmentCount; i++) {
                ...
                if (controlPointsVisibility != null && (i == segmentCount && canShowControlPoints(controlPointsVisibility, 'Target'))
                    ...
                    this.renderCircularHandle('bezierPoint_' + (i + 1) + '_2', wrapper, bezierPoint.x, bezierPoint.y, canvas, canShowCorner(selectorConstraints, 'ConnectorTargetThumb'), constraints & ThumbsConstraints.ConnectorTarget, transform, undefined, undefined, { 'aria-label': 'Thumb to move the target point of the connector' }, undefined, 'e-diagram-bezier-handle e-target', handleSize);
            ...
            if (!(group.elementActions & ElementAction.ElementIsGroup) && diagram instanceof Diagram && diagram.nameTable[group.id] && diagram.nameTable[group.id].propName !== 'connectors') {
                if (isNodeSelected && selectedNode) {
                    ...
                    if (group.flip !== 'None' && selectedNode.flipMode === 'None') {
                        if (selectedNode.shape.type === 'Text') { }
                        else {
                            this.renderFlipElement(group, innerNodeContent, group.flip);
                        }
                    }
