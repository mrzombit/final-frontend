...
var Calculate = /** @class */ (function (_super) {
                    ...
                    if (this.getLibraryFormulas().get(libFormula.toUpperCase()).isCustom) {
                        ...
                        if (sFormula.includes(this.getParseArgumentSeparator() + this.tic)) {
                            ...
                            for (var k = 0; k < args.length; k++) {
                                if (args[k] && args[k][0] === this.tic &&
                                    args[k][args[k].length - 1] !== this.tic) {
                                    joinIdx = k;
                                }
                                if (joinIdx !== null && joinIdx === k - 1 && args[k][0] !==
                                    this.tic && args[k][args[k].length - 1] === this.tic) {
                                    args[joinIdx] = args[joinIdx] + this.getParseArgumentSeparator() +
                                        args[k];
                                    args.splice(k, 1);
                                    joinIdx = null;
                                }
    ...
    Calculate.prototype.toOADate = function (dateTime) {
        var result = (dateTime.getTime() - this.oaDate.getTime()) / this.millisecondsOfaDay;
        return result;
    };
    /**
     * @hidden
    ...
    Calculate.prototype.refresh = function (cellRef, uniqueCell, dependentCell) {
        var _this = this;
        var refreshCells;
        if (!dependentCell) {
            refreshCells = true;
            ...
            try {
                ...
                if (refreshCells) {
                    ...
                    dependentCell.forEach(function (cell) {
                        ...
                        if (cellObj_1) {
                            _this.parentObject.notify('calculateFormula', { cell: cellObj_1, rowIdx: rowIdx_1, colIdx: colIdx_1, sheetIndex: sheetIdx_1, isDependentRefresh: true });
                        }
                    });
                }
            }
